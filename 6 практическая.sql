...
--Список всех заказов за определенный период времени (например, сентябрь 2010 года) отсортированный по дате заказа; (сентябрь 2010 заменён на промежуток с января по март 2016)
select * 
	from orders 
	where order_date between '2016-01-01' and '2016-03-31';

--Список всех товаров, названия которых включают слово ‘монитор’ с указанием их остатка на складе. (слово 'монитор' заменено на слово 'Jet')
select prod_name 
	from products 
...
select order_num, sum(item_price) 
	from orderitems 
	group by order_num;

--Список клиентов, которые за заданный период (например, сентябрь 2010 года) совершили более 3 заказов.(период заменён на январь-март 2016 и количество заказов более или равно 2)
select count(order_num) 
	from orders 
	where order_date between '2016-01-01' and '2016-03-31' and cust_id >= 2;